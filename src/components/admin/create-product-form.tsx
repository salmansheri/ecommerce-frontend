...
export function CreateProductForm() {
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			mutate({
				path: { categoryId: Number(value.categoryId) },
				body: {
					name: value.name,
				...
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<CardDescription className="text-zinc-500">
							Fill out the catalog information. All fields marked with * are
							required.
						</CardDescription>
					</div>
				</div>
								...
								{(field) => {
									...
									return (
										<Field data-invalid={isInvalid}>
											...
											/>
											<FieldDescription className="text-zinc-500">
												Sale price if product is discounted.
											</FieldDescription>
											{isInvalid && (
												<FieldError errors={field.state.meta.errors} />
											)}
